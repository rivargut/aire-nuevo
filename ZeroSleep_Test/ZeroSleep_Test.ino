...
*/
  ...
  // Ricardo 28 Feb 20 - Changed baud rate to 19200, init Port added per fona library changes
...
//   printlogln(F("GPS Off"));
   ...
   rtc.attachInterrupt(elResto); //Run elResto every time alarm activates
   rtc.setAlarmSeconds(rtc.getSeconds()+3); //Set the alarm in 2 seconds from now
   rtc.enableAlarm(rtc.MATCH_SS); //The alarm will compare only the seconds
   
   printlogln(F("*****  Init End  ******\r\n"));
   datafile.flush();
   logopen = false;
   USBDevice.detach(); // Ricardo 28 Feb 2020 - detaches USB port so it can restart again
   rtc.standbyMode(); //Activates standby Mode
}
...
void elResto()
{
  USBDevice.attach();
	...
	ciclos++;
	rtc.setAlarmSeconds(segundoExtra());
