...
boolean Adafruit_FONA::SIMreset(void) {
...
//#endif
  if (! sendCheckReply(F("AT+CMGF=1"), ok_reply)) {
	  DEBUG_PRINT(F("ERROR Setting SMS text mode"));
  };
  
  if (! sendCheckReply(F("AT+CSCLK=1"), ok_reply)) {
	  DEBUG_PRINT(F("ERROR Setting slow clock mode"));
  };

  // Check if charging is enabled, else set charging and reset device
  if (! sendCheckReply(F("AT+ECHARGE?"),F("+ECHARGE: 1"))) {
	  DEBUG_PRINT (F("ECHARGE not enabled. Enabling Echarge and reset SIM808"));
	  readline(); // flush OK
	  sendCheckReply(F("AT+ECHARGE=1"),ok_reply);
...
boolean Adafruit_FONA::SleepMode(uint8_t i) {
  sendCheckReply(F("AT+CSCLK="), i,ok_reply);
  sendCheckReply(F("AT&W"),ok_reply); // Ricardo 23 Feb - saving the profile and rebooting makes effective the charging mode
}
...
int8_t Adafruit_FONA::sendSMScheck() {
  uint8_t l = readline(); // read the +CMGS reply, wait up to 10 seconds!!!
  if (l >0) {
	  
	  DEBUG_PRINT("SMS check: "); DEBUG_PRINTLN(replybuffer);
	  //if (strstr(replybuffer, "+CMGS") == 0) {
	  //  return false;
	  //}
	  if (strcmp(replybuffer, "ERROR") == 0) {
		return -1;
	  }  
	  readline(200); // read OK
...
/********* Phone Book - Added by Ricardo 4 July 2020*********************/

boolean Adafruit_FONA::ReadPhonebook(uint8_t index, char *number, char *name) {
  if (!getReply(F("AT+CPBR="), index,(uint16_t) 3000))
	  return false;
  if (! parseReplyQuoted(F("+CPBR: "), number, 40,',', 1))
    return false;
  if (! parseReplyQuoted(F("+CPBR: "), name, 15,',', 3))
    return false;
...
boolean Adafruit_FONA::WritePhonebook(uint8_t index, char *number, char *name) {
...
// Separate time query to avoid having blank times as much as possible
boolean Adafruit_FONA::getGPStime(char *date) {
 ...
 if (_type == FONA808_V2) {
    ...
    return false;  // only works for SIM808 v2
  }
...
boolean Adafruit_FONA::HTTP_action(uint8_t method,uint16_t *status, uint16_t *datalen, int32_t timeout) {
...
boolean Adafruit_FONA::HTTP_GET_start(char *url,
  ...
  // HTTP GET
  if (! HTTP_action(FONA_HTTP_GET,status,datalen,20000))
    return false;
...
boolean Adafruit_FONA::HTTP_POST_start(char *url,
  ...
  // HTTP POST
  if (! HTTP_action(FONA_HTTP_POST,status,datalen,30000))
  {
...
boolean Adafruit_FONA::HTTP_POST_start_asynch(char *url,
              FONAFlashStringPtr contenttype,
              const uint8_t *postdata, uint16_t postdatalen){
  if (! HTTP_setup(url))
    return false;
  ...
  if (! expectReply(ok_reply))
	...
	// asynch HTTP in the future
	mySerial->print(F("AT+HTTPACTION="));
	mySerial->println(FONA_HTTP_POST);
	DEBUG_PRINT(F("\t---> AT+HTTPACTION="));DEBUG_PRINTLN(FONA_HTTP_POST);
	
	return true;
	
}

boolean Adafruit_FONA::HTTP_POST_start_check(uint16_t *status, uint16_t *datalen) {

	  // Parse response status and size.
	  readline();
	  DEBUG_PRINT(F("\t--->")); DEBUG_PRINTLN(replybuffer);
	  if (strcmp(replybuffer, "OK") == 0) {
		  DEBUG_PRINTLN(F("OK received"));
		  return false;
	  }
	  else if (strcmp(replybuffer, "") != 0) {
		  if (! parseReply(F("+HTTPACTION:"), status, ',', 1))
			return false;
		  if (! parseReply(F("+HTTPACTION:"), datalen, ',', 2))
			return false;
		...
		if (! HTTP_readall(datalen))
		{
			DEBUG_PRINTLN (F("\tHttppost enviado, pero se recibió un error. "));
			return true;
		}
	  }
	  else {
		DEBUG_PRINTLN(F("\tHTTP no reply yet"));
		return false;
	  }
    ...
    while(mySerial->available()) {
      ...
      if (c == 0xA) {
        ...
        if (!multiline) {
		  ...
		  //DEBUG_PRINTLN(F("Read 0xA - break"));
          break;
        }
      }
...
uint8_t Adafruit_FONA::getReply(FONAFlashStringPtr prefix, int32_t suffix, uint16_t timeout) {
  ...
  DEBUG_PRINT(F("\t1---> ")); DEBUG_PRINT(prefix); DEBUG_PRINTLN(suffix, DEC);
  ...
  DEBUG_PRINT (F("\t<---1 ")); DEBUG_PRINTLN(replybuffer);
