...
//  while (!SerialUSB);
  ...
  SerialUSB.println("*****  ECU GPS INIT ******");
  ...
  if (PM->RCAUSE.reg == PM_RCAUSE_WDT) {
   ...
   printlogln("*****  ECU GPS INIT ******");  // Init file buffer - later use strcat
   ...
   while (! fona.begin()) {
     printlogln("Couldn't find FONA - delay and retry");
	   delay(1000);
   }
   ...
   if (rtc.getYear() == 0) {
      printlogln("Delay 60 seconds for initial GPS lock"); 
      delay(60000);
      GetGPSandTime();
   }
   // Timer setup
   ...
   printlogln("*****  Init End  ******\r\n");
   sendText("ECU-GPS booted OK");
   SMStimeout = 15;
   while (SMStimeout > 0) {
      SMSstatus = fona.sendSMScheck();
      if (SMSstatus == -1) {
         SerialUSB.println(F("ERROR sending SMS"));
         SMSsent = false;
         break;
      }
      else if (SMSstatus == 1) {
        SerialUSB.println(F("SMS sent ok!"));
        SMSsent = false;
        break;
      }
      delay(1000);
...
void loop() {
  ...
  if (!engineflag && !IGN_Det) {
   if (SMSsent) {
    ...
    if (!wdtwake) {
	    printlogln("** Engine is off - going to sleep");
	    SerialUSB.end();
    }
	  LowPower.sleep();
    if (!wdtwake) {
	    ...
	    printlogln("\r\n** Interrupt triggered and wake CPU");
    }
  }
  if (!wdtwake) {
  	ReadOBD();
...
void ReadOBD() {
  ...
  if (IGN_Det == false) {
    ...
    if (SIMflag == false) {
	  ...
	  if (SMSsent) {
      printlogln("Wait for SMS to be sent");
      delay(500);
	  }
    else {
  	  BattMeasure();
      ...
      if (engineflag == true) { // Ricardo 24 March 2020 - include an engine on check since car could have turned off
        ...
        sendText("Engine is turned OFF");
        lastpost = true;
        postflag = true;
        engineflag = false;
        // Insert HTTP posting here immediately after turn off. Need to change posting flags
        //HTTPPost();
      }
    ...
    if (engineflag == false) {
      ...
      digitalWrite(LIN_SLP, HIGH);
  	  delay(50);
  	  SIMflag = true;
	    if (SMSsent || HTTPsending) {
        printlogln("Fona is busy, wait to use SIM");
	    }
      else{
  ...
  if ((obdflag == ISO9141SLOW) || (obdflag == ISO9141KWP) || (obdflag == KWPSUZ)) {
	...
	// RPM reading is valid!
		...
		if (fRPM == 0) {
		  ...
		  printlogln("Ignition is ON but Engine is off - skip calculation");
    ...
    if (bFuelType) {
      ...
      fLPGKm += fSpeed * elapsedmillis/3600000;      // added cumulative of Km instead of Km/l 10/12/2020
      ...
      fGasKm += fSpeed * elapsedmillis/3600000;      // added cumulative of Km instead of Km/l 10/12/2020
    ...
    sprintf(stringbuffer,"%4.0f\t%.0f\t%.0f\t%.0f\t%.2f\t%d\t%.2f\t%.3f\t%.3f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f",fRPM,fSpeed,fIMAP,fIAT,fAir,elapsedmillis,fLPH,fGasKm,fLPGKm,fGasTime,fLPGTime,fGasPerc,fLPGPerc,fGasLiters,fLPGLiters,fAvgGasLPH,fAvgLPGLPH);
    printlogln(stringbuffer);
...
void ProcessSMS () {
  ...
  if (SIMflag == false) {
	...
	/*
    printlogln("Turn ON SIM808 via PWRKEY");
    SimTogglePWRKEY();
     if (! fona.begin()) {
          printlogln("Couldn't find FONA");
          delay(5000);
		      return;
     }
     else{
        printlogln("Turn ON SIM808 successfully");
        printlogln("30 seconds Delay before reading SMS");
        SerialUSB.end();
        LowPower.sleep(30000);
        // Resume from Wake-up
        SerialUSB.begin(115200);
        delay(2000);
		printlogln("Wake up OK");
        ...
        printlogln(stringbuffer);
		  ...
		  if (smsfound) {
			  ...
			  if (strcmp(command,"PASS") == 0) {
                ...
                SMStimeout = 15;
                SMSstatus = 0;
    					  printlogln("SMS sent with password confirmation");
    					}
  				  }
  				}
			  }
            ...
            SMSstatus = 0;
				...
				if (i != 0) {
				  printlogln("WARNING Password sent is NOT correct.. bypassing action");
					...
					if (GPSflag == false) {
						if (!fona.enableGPS(true)) {
							...
							digitalWrite(PWR_GPS,LOW);
							printlogln("GPS Enabled! Waiting 60 seconds for GPS lock");
							delay(60000);
				  ...
				  if (fona.getGPS(&lat,&lon,&speedkph,&heading,&altitude) ) {
					  sprintf(stringbuffer,"Latitude: %f, Longitude: %f",lat,lon);
					  printlogln(stringbuffer);
					  lastlat = lat;
					  lastlon = lon;
				  }
            ...
            SMStimeout = 15;
            SMSstatus = 0;
					  printlogln("SMS sent with GPS coordinates");
				  }
				}
			  }
              ...
              SMStimeout = 15;
              SMSstatus = 0;
  					  printlogln("SMS sent for Shutdown");
  				  }
              ...
              SMStimeout = 15;
              SMSstatus = 0;
  					  printlogln("SMS sent for Reconnect");
  				  }
  				}
			  }
              ...
              SMStimeout = 15;
              SMSstatus = 0;
							printlogln("SMS sent with Battery Voltage");
						}
            ...
            psms.print(fLPGKm);
              ...
              SMStimeout = 15;
              SMSstatus = 0;
							printlogln("SMS sent with Parametric data");
						}
               ...
               obdflag = 7;
						...
						printlog("Confirm sender ");
						printlogln(sender);
						if (!fona.sendSMS(sender, "New Parameter OK")) {
						  printlogln("ERROR could not send SMS");
						} else {
              SMSsent = true;
              SMStimeout = 15;
              SMSstatus = 0;
						  printlogln("SMS sent with parameter confirmation");
						}
...
void sendText(char *msg)
{
  ...
  if (!HTTPsending) {
    rtctimestamp(timestamp);
    printlogln(msg);
    psms.print(timestamp);
    psms.print(": ");
    psms.print(msg);
  	if (!fona.sendSMS("84050685", sms)) {
  		printlogln("ERROR could not send SMS");
    } else {
          SMSsent = true;
          SMStimeout = 15;
          SMSstatus = 0;
    	    printlogln("SMS sent with message");
    }
  }
}
...
void SDWriteFile() { 
    ...
    if (digitalRead(SD_DT) == HIGH) {
      ...
      if (SD.begin(SD_CS)) {
		...
		SerialUSB.print("Writing data buffer to file.. ");
...
void wdt_reset() {
  digitalWrite(DTR,LOW);
  delay(100);
  sendText("System reset by watchdog!");
}
...
void WDT_Handler(void) {  // ISR for watchdog early warning, DO NOT RENAME!, need to clear
  ...
  wdtwake = true;                            // Skip rest of wakeup cycle if its a WDT early warning
